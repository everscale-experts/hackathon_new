...
pub async fn runget_command(matches: &ArgMatches<'_>, config: Config) -> Result<(), String> {
    let address = matches.value_of("ADDRESS");
    let method = matches.value_of("METHOD");
    let params: Option<std::slice::Iter<&str>> = Some([].iter());
    // let params = Some(["arg1", "arg2"].iter());
    let params = params.map(|values| {
        serde_json::json!(values.collect::<Vec<_>>()).to_string()
    });
    if !config.is_json {
