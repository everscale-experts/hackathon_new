...
pub fn json_account(
    ...
    if last_paid.is_some() {
        res["last_paid"] = serde_json::json!(last_paid.unwrap());
    }
    if last_trans_lt.is_some() {
        res["last_trans_lt"] = serde_json::json!(last_trans_lt.unwrap());
    }
    if data.is_some() {
        res["data(boc)"] = serde_json::json!(data.unwrap());
    }
    if code_hash.is_some() {
        res["code_hash"] = serde_json::json!(code_hash.unwrap());
    }
    if state_init.is_some() {
        res["state_init"] = serde_json::json!(state_init.unwrap());
    }
    res
}
