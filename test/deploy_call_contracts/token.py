...
class FA12_core(sp.Contract):
        ...
        token_metadata = sp.big_map(
            l = {
                0: (
                    ...
                    sp.map(
                        l = {
                            "name": sp.bytes_of_string('EVERTZ'),
                            "decimals": sp.bytes_of_string('6'),
                            "symbol": sp.bytes_of_string('MGT'),
                            "icon": sp.bytes_of_string('https://smartpy.io/static/img/logo-only.svgg')
                        },
                        tkey = sp.TString,
                        tvalue = sp.TBytes
                    )
    ...
    def transfer(self, params):
        ...
        sp.verify(self.data.balances[params.from_].balance >= params.value, "InsufficientBalance")
...
class FA12_mint_burn(FA12_core):
    ...
    def mint(self, params):
        ...
        sp.verify(self.is_administrator(sp.sender), "NotAdministrator")
    ...
    def burn(self, params):
        sp.set_type(params, sp.TRecord(address = sp.TAddress, value = sp.TNat))
        sp.verify(self.is_administrator(sp.sender), "NotAdministrator")
        sp.verify(self.data.balances[params.address].balance >= params.value, "InsufficientBalance")
